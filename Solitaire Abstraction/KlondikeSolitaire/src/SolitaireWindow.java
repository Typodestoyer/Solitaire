...
public class SolitaireWindow extends JFrame
{
	...
	private static final int PILES_X = 100;
	private static final int PILES_Y = 200;
	private static final int PILES_X_GAP = 100;
	private static final int PILES_Y_GAP = 30;
	private static final int TARGETS_X = 350;
	private static final int TARGETS_X_GAP = 125;
	private static final int DISCARD_X = 150;
	...
	public void initDisplay()
	{
		...
		for(int i = 0; i < game.NUM_OF_CARDS_DEALT; i ++)
		{
			...
			panel.add(discardDisplay.get(i));
		...
		for(int i = 0; i < 7; i ++)
		{
			...
			for(int j = 0; j < acp.size(); j ++)
			{
				...
				panel.add(pilesDisplay.get(i).get(j));
				if(j != acp.size() - 1)
				{
		...
		for(int i = 0; i < 4; i ++)
		{
			...
			panel.add(targetsDisplay.get(i));
		}
			...
			AlternatingCardPile acp = game.getPiles()[i];
			{
				for(int j = 0; j < acp.size() || j < pilesDisplay.get(i).size(); j ++)
				{
					if(j >= acp.size())
					{
						...
						{
							panel.remove(pilesDisplay.get(i).get(j));
					...
					//Current issue: mouseListener doesn't recognize what I'm clicking for whatever reason.
				}
			}
		}
